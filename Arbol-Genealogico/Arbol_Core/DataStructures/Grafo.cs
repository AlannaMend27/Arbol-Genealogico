    ...
    public class Grafo
    {
        ...
        public double CalcularDistanciaPromedio()
        {
            ...
            for (int i = 0; i < lista.Count; i++)
            {
                for (int j = i + 1; j < lista.Count; j++)
                {
                    // Calcular distancia euclidiana en píxeles (igual a MapaUI)
                    double dx = lista[i].Longitud - lista[j].Longitud;
                    double dy = lista[i].Latitud - lista[j].Latitud;
                    double distancia = System.Math.Sqrt(dx * dx + dy * dy);
        ...
        public (string distancia, string lejos, string cerca) ObtenerValoresUI()
        {
            ...
            string distanciaTexto = $"{CalcularDistanciaPromedio():F2} px";
